...
def generate_content():
    try:
        ...
        if user_link and user_link.strip():
            link_instruction = f"4. في نهاية الإعلان وتحديداً عند دعوة اتخاذ الإجراء، ضع رابط العميل الحقيقي التالي بشكل مدمج وجذاب ليتواصل الناس معه من خلاله: {user_link}"
        else:
            link_instruction = "4. في نهاية الإعلان وتحديداً عند دعوة اتخاذ الإجراء، ضع عبارة تسويقية عامة تطلب من الزبائن التواصل مباشرة مع صفحة المتجر."

        prompt = f"""
        أنت خبير تسويق رقمي وصانع محتوى محترف لصالح منصة "AkramFlow AI".
        قم بإنشاء محتوى تسويقي عالي الجودة ومقنع وفصل الفقرات بعناية بناءً على التالي:
        ...
        3. تحذير صارم: لا تكتب أقواس روابط فارغة أو نصوص مبهمة مثل "اضغط هنا للرابط".
        {link_instruction}
