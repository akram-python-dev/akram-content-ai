...
def generate_content():
    try:
        ...
        user_link = data.get('userLink', '')  # استقبال رابط متجر المستخدم الجديد
        ...
        # تجهيز نص الرابط إذا قام المستخدم بإدخاله
        link_instruction = ""
        if user_link:
            link_instruction = f"4. في نهاية الإعلان وتحديداً عند دعوة اتخاذ الإجراء (Call to Action)، ضع رابط العميل الحقيقي هذا ليتواصل الناس معه من خلاله: {user_link}"
        else:
            link_instruction = "4. في نهاية الإعلان وتحديداً عند دعوة اتخاذ الإجراء (Call to Action)، ضع عبارة عامة تطلب من الزبائن التواصل مع صفحة المتجر."
        ...
        3. تحذير صارم: لا تكتب أبداً جمل نصية مثل "اضغط هنا" كروابط عامة، بل التزم بالرابط الموفر لك فقط إن وُجد.
        {link_instruction}
        5. عند نهاية المحتوى بالكامل (بعد الرابط والخاتمة)، قم بإضافة السطر التالي والمنفصل تماماً كعلامة مشفرة لظهور زر تواصل العميل معك أنت كصاحب المنصة:
        [SHOW_CONTACT_BUTTON]
